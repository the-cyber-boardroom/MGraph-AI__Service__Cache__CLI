...
git fetch template
git merge template/main --allow-unrelated-histories -m "Initial import from MGraph-AI__Service__Cache__CLI template"
print_success "Template merged"

# Step 5: Rename the service directory
print_step "Renaming service directory from mgraph_ai_service_cache__cli to $SERVICE_NAME_UNDERSCORES..."
if [ -d "mgraph_ai_service_cache__cli" ]; then
    mv mgraph_ai_service_cache__cli "$SERVICE_NAME_UNDERSCORES"
    print_success "Service directory renamed"
else
    print_error "mgraph_ai_service_cache__cli directory not found!"
    exit 1
fi
...
# Replace "MGraph AI Service Cache CLI" with the new service title
replace_in_files "MGraph AI Service Cache CLI" "$SERVICE_NAME_TITLE" "service title"

# Replace "mgraph_ai_service_cache__cli" with the new package name
replace_in_files "mgraph_ai_service_cache__cli" "$SERVICE_NAME_UNDERSCORES" "Python package name"

# Replace "MGraph-AI__Service__Cache__CLI" with the new repository name
replace_in_files "MGraph-AI__Service__Cache__CLI" "$REPO_NAME" "repository name"
...
- Based on MGraph-AI__Service__Cache__CLI v0.3.0
